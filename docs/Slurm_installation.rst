...
Set the version (for example, 25.11.3 and build Slurm_ RPM packages by::
...
In our setup we build packages with these options::
  ...
  rpmbuild -ta slurm-25.11.3.tar.bz2 --with slurmrestd --with freeipmi --with pmix
...
9. At this point you have a Slurm_ database server running an exact copy of your main Slurm_ database!
   ...
   Update all Slurm_ RPMs to the new version (say, 25.11.3 built as shown above)::
...
The **quick and usually OK procedure** would be to simply update the RPMs (here: version 25.11.3 on all nodes::

  clush -bw <nodelist> 'dnf -y update /some/path/slurm*25.11.3*.rpm'
...
3. Update the RPMs (here: version 25.11.3 on nodes::
